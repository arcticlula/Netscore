...
extern volatile uint8_t current_mux = 0;
extern volatile uint8_t target_mux = 0;
volatile uint8_t active_buffer = 0;
volatile uint8_t inactive_buffer = 1;
uint8_t mux[] = {MUX_DD_1, MUX_DD_2, MUX_SD};

float segment_a[6] = SEGMENT_A_VALUES;
float segment_b[6] = SEGMENT_B_VALUES;

gpio_num_t mux_1, mux_2, mux_3;

volatile gpio_num_t mux_sel;
...
void Tlc5940::init(uint8_t sin, uint8_t sout, uint8_t sclk, uint8_t xlat, uint8_t blank, uint8_t gsclk, uint8_t vprg, uint8_t dcprg, uint8_t xerr) {
  init_mux(MUX_DD_1, MUX_DD_2, MUX_SD);
  /* Pin Setup */
  ...
  // Configure LEDC for GSCLK
  ledc_timer_config_t ledc_timer = {};
  ledc_timer.speed_mode = LEDC_LOW_SPEED_MODE;
  ledc_timer.duty_resolution = LEDC_TIMER_5_BIT;
  ledc_timer.timer_num = LEDC_TIMER_0;
  ledc_timer.freq_hz = 2000000;  // 2MHz frequency
  ledc_timer.clk_cfg = LEDC_USE_APB_CLK;

  ESP_ERROR_CHECK(ledc_timer_config(&ledc_timer));

  ledc_channel_config_t ledc_channel = {};
  ledc_channel.gpio_num = gsclk_pin;
  ledc_channel.speed_mode = LEDC_LOW_SPEED_MODE;
  ledc_channel.channel = LEDC_CHANNEL_0;
  ledc_channel.intr_type = LEDC_INTR_DISABLE;
  ledc_channel.timer_sel = LEDC_TIMER_0;
  ledc_channel.duty = 8;  // 25% duty cycle (8 / 32)
  ...
  config.resolution_hz = 1000000;  // 1MHz
  ...
  ESP_LOGI(TAG, "Attempting timer start...");
...
void display_update_task(void *arg) {
  while (1) {
    ...
    // Send all data in one transaction for the active mux and buffer
    spi_send(gs_buffers[active_buffer][current_mux], NUM_TLCS * 24);
    xlatNeedsPulse = 1;

    set_mux((gpio_num_t)mux[current_mux]);
