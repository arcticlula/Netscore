...
/*
    ...
    case PRACTICE_HOME_WIN_SCR:
      show_practice_result(HOME);
      break;
    case PRACTICE_AWAY_WIN_SCR:
      show_practice_result(AWAY);
      break;
...
/*
    case SPORT_PRACTICE:
      show_text(SIDE_BOTH, T, R, E, I, N, O, menu_brightness);
      break;
*/
    case SPORT_VOLLEY:
...
/*
void show_practice() {
  uint32_t time_to_show = 4000000;
  uint64_t t_now = esp_timer_get_time();

  if (score.last_point_timestamp > 0 && t_now - score.last_point_timestamp < time_to_show &&
      !match.history.empty() && match.history.back().timestamp == score.last_point_timestamp) {
    set_symbol(&dw3.c, score.last_home_balls);
    set_symbol(&dw4.c, score.last_away_balls);
    show_wave(SIDE_A, DIGIT_3, &dw3);
    show_wave(SIDE_A, DIGIT_4, &dw4);
  } else {
    show_symbol(SIDE_A, DIGIT_3, score.home_balls, 50);
    show_symbol(SIDE_A, DIGIT_4, score.away_balls, 50);
  }

  bool home_score_point = score.home_balls == 1 && score.away_balls < 2;
  bool away_score_point = score.away_balls == 1 && score.home_balls < 2;

  // If next point is set point, show dot
  if (home_score_point) show_dot(SIDE_A, DIGIT_2, &dd1);
  if (away_score_point) show_dot(SIDE_A, DIGIT_6, &dd2);
...
void show_practice_result(uint8_t team) {
  uint8_t set_idx = score.home_sets + score.away_sets;
...
void show_practice_end() {
  static uint8_t score_counter = 0;
  static bool run = true;
  // Center digits show options zigzag

  // Left Option: Continue Practice
  if (practice_end_option.current == FIRST) {
    show_zigzag(SIDE_BOTH, DIGIT_3, &dz1);

    // Animate left side: loop counter to 3 + show score
    uint32_t t = esp_timer_get_time() / 1000;
    uint8_t step = (t / 500) % 4;  // 0, 1, 2, 3
    ...
    show_symbol(SIDE_BOTH, DIGIT_1, step, 50);
    show_number(SIDE_BOTH, DIGIT_2, score_counter % 10, 50);
  } else {
    // Right Option: Start New Game
    show_zigzag(SIDE_BOTH, DIGIT_4, &dz1);

    // Animate right side: simply counting up
    static uint32_t next_increment_time = 0;
    uint32_t t = esp_timer_get_time() / 1000;

    if (t >= next_increment_time) {
      score_counter = (score_counter + 1) % 25;
      next_increment_time = t + 500 + (esp_random() % 501);
    }

    show_character(SIDE_BOTH, DIGIT_5, numbers[score_counter / 10], 50);
    show_character(SIDE_BOTH, DIGIT_6, numbers[score_counter % 10], 50);
  }
}
*/
...
void show_test() {
  bool digit_l = false;  // BUTTON_LEFT_PIN is no longer defined
