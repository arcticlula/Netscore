...
void saveSettings() {
  ...
  SystemSettings settings = {display_mode, brightness_index};
  ...
  if (err == ESP_OK) {
    nvs_commit(my_handle);
    ESP_LOGI(TAG, "Settings saved: Mode %d, Brightness %d", settings.display_mode, settings.brightness_index);
  }
  nvs_close(my_handle);
}
...
void loadSettings() {
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGW(TAG, "NVS open failed for settings.");
  }
}
