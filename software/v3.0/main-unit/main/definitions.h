...
// --- Hardware pin map: Netscore main board v3.0 ---
// See hardware/docs/pins.md (v3.0 table); netlist-verified against
// pcb/v3.0/main-board/main-board.kicad_sch.

#include "driver/gpio.h"    // gpio_num_t + pin macros
#include "driver/ledc.h"    // LEDC_CHANNEL_* (status LED)
#include "hal/adc_types.h"  // ADC_CHANNEL_* for ADC_BAT_PIN

// What changed from v2.0, for orientation:
//   - dot-correction chain removed from the slot connector (DCSIN is strapped
//     high locally on the display boards, so DEFAULT_DCSIN_PIN is -1)
//   - chain return added: GSSOUT reads back through the U4 switch on GPIO13
//   - rotary encoder replaces the three nav buttons
//   - DRV8662 piezo driver replaces the DRV8833 H-bridge buzzer
...
// Status LED on GPIO45. The buzzer uses SDM+GPTimer, so LEDC is otherwise
// free on this board — no channel collision.
#define BOARD_LED_LEDC_CHN LEDC_CHANNEL_1
#define BOARD_LED_LEDC_TIMER LEDC_TIMER_2
...
typedef struct {
  device_t device_id;
  uint8_t window;
  uint8_t overlay_window;
  uint8_t menu;
  uint8_t sport;
  uint8_t game_mode;       // sport_mode_t
  uint8_t generic_option;  // max score index, padel option index, practice toggle, etc.
...
#define BIG_GREEN_RED BIG_PURE_GREEN, BIG_PURE_GREEN, BIG_RED, BIG_RED
#define TIME_BLUE SMALL_BLUE, SMALL_BLUE, SMALL_BLUE, SMALL_BLUE
#define SETS_GREEN_RED SMALL_GREEN, SMALL_RED
