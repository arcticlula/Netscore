...
void show_display() {
  ...
  uint32_t time_diff = esp_timer_get_time() - last_interaction_time;

  if (window == PLAY_MENU_SCR && (time_diff > 3000000LL)) {
    init_play_scr();
  }

  if (window == PLAY_MENU_PAINEL_SCR && (time_diff > 2000000LL)) {
    init_play_menu_scr();
  }

  if (overlay_window_active && (time_diff > 1000000LL)) {
    overlay_window_active = false;
    save_type_t save_msg = SAVE_SETTINGS;
...
void show_set_padel_game_type() {
  ...
  if (padel_game_type_option.current == FIRST) {
    if (inf_patern_a) show_zigzag(SIDE_BOTH, POINTS_HOME_1, &dz[POINTS_HOME_1], change_pattern_a);
      ...
      show_letter(SIDE_BOTH, POINTS_HOME_1, BLANK, 30);
    if (inf_patern_b) show_zigzag(SIDE_BOTH, POINTS_HOME_2, &dz[POINTS_HOME_2], change_pattern_b);
    else
      show_letter(SIDE_BOTH, POINTS_HOME_2, BLANK, 30);
