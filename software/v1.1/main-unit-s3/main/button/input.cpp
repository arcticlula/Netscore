...
void init_inputs(void) {
  for (int i = 0; i < NUM_OF_INPUTS; i++) {
    ...
    timer_args[i] = {};
    timer_args[i].callback = &timer_handler;
    timer_args[i].arg = &input_indices[i];
    timer_args[i].dispatch_method = ESP_TIMER_TASK;
    timer_args[i].name = "timer";

    timer_debounce_args[i] = {};
    timer_debounce_args[i].callback = &timer_debounce_handler;
    timer_debounce_args[i].arg = &input_indices[i];
    timer_debounce_args[i].dispatch_method = ESP_TIMER_TASK;
    timer_debounce_args[i].name = "timer debounce";

    timer_hold_args[i] = {};
    timer_hold_args[i].callback = &timer_hold_handler;
    timer_hold_args[i].arg = &input_indices[i];
    timer_hold_args[i].dispatch_method = ESP_TIMER_TASK;
    timer_hold_args[i].name = "timer hold";
