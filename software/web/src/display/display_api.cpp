...
void show_text(
    uint8_t side, uint8_t l_1, uint8_t l_2, uint8_t l_3, uint8_t l_4, uint8_t l_5, uint8_t l_6, uint8_t value) {
  switch (current_mux) {
    case 0:
      // 1
...
void show_wave(uint8_t side, uint8_t offset_ch, digit_wave_t *digit, void (*callback)()) {
  double time = digit->time_ms / (2 * MUX_NUM);  // only called every x time, x is the number of
  ...
  for (int channel = 0; channel < 8; channel++) {
    ...
    Tlc.set(offset_ch + channel, set_value, side);
  }
...
void show_zigzag(uint8_t side, uint8_t offset_ch, digit_zigzag_t *digit, void (*callback)()) {
  ...
  uint16_t background = round(digit->background);
  double time = digit->time_ms / (2 * MUX_NUM * digit->c.size);  // only called every x time, x is the number of
                                                                 // mux outputs, the 2 is bc only called every 2ms
...
void show_fade_in(uint8_t side, uint8_t offset_ch, digit_fade_t *digit, void (*callback)()) {
  static double time =
      digit->time_ms / (2 * MUX_NUM * digit->c.size);  // only called every x time, x is the number of mux
