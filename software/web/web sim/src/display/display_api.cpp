...
void show_character(uint8_t side, uint8_t logical_digit, uint8_t character, uint8_t val) {
  ...
  for (uint8_t channel = 0; channel < 7; channel++) {
    ...
    Tlc.setSegment(logical_digit, channel, on == 1 ? value : 0);
  }
}
...
void show_wave(uint8_t side, uint8_t logical_digit, digit_wave_t *digit, void (*callback)()) {
  double time = digit->time_ms / FRAME_TIME_MS;  // only called every x time, x is the number of
                                                 // mux outputs, the 2 is bc only called every 2ms
  ...
  for (int channel = 0; channel < 8; channel++) {
    ...
    uint16_t set_value = on == 1 ? value : background;
    Tlc.setSegment(logical_digit, channel, set_value);
  }
  ...
  // printf("Value: %d, Ratio: %.2f, Direction: %d\n", digit->value, ratio, digit->direction);
}
...
void show_zigzag(uint8_t side, uint8_t logical_digit, digit_zigzag_t *digit, void (*callback)()) {
  ...
  uint16_t background = round(digit->background);
  double time = digit->time_ms / (FRAME_TIME_MS * digit->c.size);  // only called every x time, x is the number of
                                                                 // mux outputs, the 2 is bc only called every 2ms
  ...
  for (uint8_t i = 0; i < digit->c.background_size; i++) {
    Tlc.setSegment(logical_digit, digit->c.background_positions[i], background);
  }

  // Set the main channel to maximum brightness
  Tlc.setSegment(logical_digit, digit->c.positions[digit->channel], max);

  // Optional: Set adjacent channels to lower brightness for smooth transition
  if (digit->direction == 1 && digit->channel > 0) {
    Tlc.setSegment(logical_digit, digit->c.positions[digit->channel - 1], min);
  }
...
void show_fade_in(uint8_t side, uint8_t logical_digit, digit_fade_t *digit, void (*callback)()) {
  static double time =
      digit->time_ms / (FRAME_TIME_MS * digit->c.size);  // only called every x time, x is the number of mux
                                                       // outputs, the 2 is bc only called every 2ms
  ...
  if (digit->channel < digit->c.size - 1) {
    ...
    if ((digit->positions_value[digit->c.size - 1] >= digit->value) && callback)
      callback();
    else
      digit->cnt++;
  }
  ...
  for (uint8_t channel = 0; channel <= digit->channel; channel++) {
    ...
    Tlc.setSegment(logical_digit, digit->c.positions[channel], value);
...
void show_fade_into(uint8_t side, uint8_t logical_digit, digit_fade_into_t *digit, void (*callback)()) {
  ...
  for (uint8_t channel = 0; channel < 8; channel++) {
    ...
    Tlc.setSegment(logical_digit, channel, round(value));
    if (direction != 0) {
...
void show_dot(uint8_t side, uint8_t logical_digit, uint8_t value) {
  ...
  Tlc.setSegment(logical_digit, 7, val);
}
...
void show_dot(uint8_t side, uint8_t logical_digit, digit_dot_t *dot) {
  ...
  Tlc.setSegment(logical_digit, 7, dot->value);
}
...
void set_chars_fade_into(digit_fade_into_t *digit, uint8_t character, uint8_t character2) {
  ...
  // uint16_t diff = pos_diff ? digit->target_value - digit->value :
  // digit->value - digit->target_value;
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    if (c1)
      digit->positions_value[i] = digit->value;
    else
