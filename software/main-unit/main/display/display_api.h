...
void show_letter(uint8_t side, uint8_t digit_index, uint8_t letter, uint8_t value);
...
void init_single_wave(single_wave_t *d, uint8_t value, uint8_t min, uint8_t max, int8_t direction, uint16_t time_ms, uint16_t time_down_ms = 0, bool exponential_down = false);
