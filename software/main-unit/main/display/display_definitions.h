#pragma once
#include "definitions.h"

typedef struct {
  uint8_t character;
  uint8_t size;
  uint8_t background_size;
  uint8_t positions[8];
  uint8_t background_positions[8];
} digit_character_t;

typedef struct {
  digit_character_t c;
  int8_t direction;
  double value;
  double min;
  double max;
  double background;
  uint32_t time_ms;
} digit_wave_t;

typedef struct {
  int8_t direction;
  double value;
  double min;
  double max;
  uint32_t time_ms;
  uint32_t time_down_ms;
  bool exponential_down;
  uint16_t cnt;
} single_wave_t;

typedef struct {
  digit_character_t c;
  int8_t channel;
  int8_t direction;
  double min;
  double max;
  double background;
  uint32_t time_ms;
  uint16_t cnt;
} digit_loop_t;

typedef struct {
  digit_character_t c;
  int8_t channel;
  int8_t direction;
  double min;
  double max;
  double background;
  uint32_t time_ms;
  uint16_t cnt;
} digit_zigzag_t;

typedef struct {
  digit_character_t c;
  int8_t channel;
  int8_t direction;
  double value;
  uint32_t time_ms;
  uint16_t cnt;
  double positions_value[8];
} digit_fade_t;

typedef struct {
  digit_character_t c1;
  digit_character_t c2;
  double value;
  uint32_t time_ms;
  uint16_t cnt;
  double positions_dir[8];
  double positions_value[8];
} digit_fade_into_t;

typedef struct {
  int8_t direction;
  double value;
  double min;
  double max;
  uint32_t time_ms;
} digit_dot_t;

#define MAX_SCROLL_TEXT_LEN 32

typedef struct {
  uint8_t text[MAX_SCROLL_TEXT_LEN];
  uint8_t text_len;
  uint8_t display_digits[10];
  uint8_t num_display_digits;
  int8_t direction;
  uint16_t time_ms;
  uint16_t cnt;
  int16_t offset;
  uint8_t brightness;
  uint8_t padding;
} text_scroll_t;

typedef struct {
  uint8_t text[MAX_SCROLL_TEXT_LEN];
  uint8_t text_len;
  uint8_t display_digits[10];
  uint8_t num_display_digits;
  int8_t direction;
  uint16_t time_ms;
  uint16_t cnt;
  int16_t offset;
  uint8_t sub_offset;
  uint8_t brightness;
  uint8_t padding;
} text_matrix_t;

// Digits
extern digit_wave_t dw[10];
extern single_wave_t wc1, wc2;
extern single_wave_t wbl[4];
extern single_wave_t wled[8];
extern digit_zigzag_t dz[10];
extern digit_loop_t dl[10];
extern digit_fade_t df[10];
extern digit_fade_into_t dfi[10];
extern digit_dot_t dd1, dd2;
extern text_scroll_t ts;
extern text_matrix_t tm;

// Logical Display Groups (Maps to TLC5951 color groups)
typedef enum {
  GROUP_POINTS = 0,  // R group - big 7-seg
  GROUP_TIME,        // G group - small 7-seg
  GROUP_SETS,        // B group mux 0-1 - 7-seg
  GROUP_LEDS,        // B group mux 2 - indicator LEDs
  GROUP_BAR          // B group mux 3 - bar LED
} display_group_t;

// Points digits (R group, 7-seg)
#define POINTS_HOME_1 0  // mux 0, tens
#define POINTS_HOME_2 1  // mux 1, units
#define POINTS_AWAY_1 2  // mux 2, tens
#define POINTS_AWAY_2 3  // mux 3, units

// Time digits (G group, 7-seg)
#define TIME_1 4  // mux 0, hours tens
#define TIME_2 5  // mux 1, hours units
#define TIME_3 6  // mux 2, minutes tens
#define TIME_4 7  // mux 3, minutes units

// Sets digits (B group, 7-seg)
#define SETS_HOME 8  // mux 0, home sets
#define SETS_AWAY 9  // mux 1, away sets
#define END_FRAME 0xFF

// Indicator LEDs (B group, mux 3)
#define LED_HOME_1 10
#define LED_HOME_2 11
#define LED_HOME_3 15
#define LED_MID 16
#define LED_AWAY_1 14
#define LED_AWAY_2 13
#define LED_AWAY_3 12
#define LED_TEST_1 17

// Time colon and bar LEDs (B group, mux 4)
#define TIME_COLON_TOP 20
#define TIME_COLON_BOTTOM 21
#define BAR_LED_1 22
#define BAR_LED_2 24
#define BAR_LED_3 25
#define BAR_LED_4 23
#define LED_TEST_2 26
#define LED_TEST_3 27

#define POINTS_GP_1 0
#define POINTS_GP_2 1
#define SETS_GP 2

#define LEDS_GP 0

typedef enum {
  LEDS_LEFT = 0,
  LEDS_RIGHT,
  LEDS_CENTER,
  LEDS_TEST
} led_group_t;