...
void init_buzzer() {
  board_buzzer_init();  // per-revision tone hardware (LEDC pairs / SDM+GPTimer)
...
void play_note(uint8_t side, uint16_t frequency, uint8_t octave, uint8_t volume_percent) {
  ...
  board_buzzer_tone(side, adjusted_frequency, adjusted_volume);
}
...
uint16_t adjust_volume_for_frequency(uint16_t raw_volume, uint16_t frequency) {
  // Board-specific amplitude floor (v2.0 DRV8833 min pulse width; v3.0 none).
  return board_buzzer_min_amplitude(raw_volume, frequency);
}
...
void melody_task(void* arg) {
  ...
  while (1) {
    if (xQueueReceive(melody_queue, &current_note, drv_awake ? pdMS_TO_TICKS(50) : portMAX_DELAY) == pdTRUE) {
      if (!drv_awake) {
        board_buzzer_wake();  // per-revision enable + settle time
        drv_awake = true;
      }
      ...
      if (current_note.note != NONE && play_duration > 0) {
        if (!current_note.is_glide) {
          ...
          board_buzzer_tone(SIDE_BOTH, freq1, vol1);
          ...
          while (1) {
            ...
            board_buzzer_tone(SIDE_BOTH, current_freq, current_volume);
