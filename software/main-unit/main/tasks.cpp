...
void init_tasks(void) {
  ...
  mirror_action_queue = xQueueCreate(10, sizeof(esp_now_msg_t));
  ...
  xTaskCreate(mirror_action_task, "mirror_action_task", 6144, NULL, 5, &mirror_action_task_handle);
  xTaskCreate(espnow_tx_task, "espnow_tx_task", 3072, NULL, 7, NULL);
  ...
  // Board input beyond plain buttons (v3.0 rotary encoder). Started here,
  // after button_action_queue exists, because the encoder task posts to it.
  // No-op on v2.0.
  board_input_init();
